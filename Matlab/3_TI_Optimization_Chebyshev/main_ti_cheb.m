...
% *** 離散化用のパラメータ ***
nk   = 3;    % グリッドの数
kmax = 0.5; %kss*1.2;   % 資本グリッドの最大値
%kmax = 0.4;   % 資本グリッドの最大値(固定資本減耗=1.0の場合、kmax=0.5にすると59行目の初期値では解けない)
%kmax = 10.0; % 資本グリッドの最大値(固定資本減耗=0.08の場合に使用)
kmin = 0.05; %kss*0.8;  % 資本グリッドの最小値 (0にすると生産が出来なくなる)
kmax = kss*1.2;   % 資本グリッドの最大値
kmin = kss*0.8;  % 資本グリッドの最小値 (0にすると生産が出来なくなる)
%========================
...
plot(kgrid0, p_true, '--', 'color', 'red', 'linewidth', 3);
...
% err2 = csvread("../1_TI_Optimization/err_ndp.csv");
% figure;
